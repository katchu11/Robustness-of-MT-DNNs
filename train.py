...
def data_config(parser):
    ...
    parser.add_argument('--data_dir', default='data/canonical_data/bert_uncased')
...
def main():
    ...
    for dataset in args.train_datasets:
        ...
        train_path = data_dir + "/" + "{}_train.json".format(dataset)
    ...
    for dataset in args.test_datasets:
        ...
        dev_path = data_dir + "/" + "{}_dev.json".format(dataset)
        dev_data = None
        ...
        test_path = data_dir+ "/"+"{}_test.json".format(dataset)
        test_data = None
    ...
    for epoch in range(0, args.epochs):
        ...
        for i in range(len(all_indices)):
            ...
            if args.save_per_updates_on and ((model.local_updates) % (args.save_per_updates * args.grad_accumulation_step) == 0):
                model_file = output_dir + "/"+ 'model_{}_{}.pt'.format(epoch, model.updates)
        ...
        for idx, dataset in enumerate(args.test_datasets):
            ...
            if dev_data is not None:
                ...
                score_file = output_dir + "/" + "{}_dev_scores_{}.json".format(dataset, epoch)
                results = {'metrics': dev_metrics, 'predictions': dev_predictions, 'uids': dev_ids, 'scores': scores}
                dump(score_file, results)
                official_score_file = output_dir + "/" + "{}_dev_scores_{}.tsv".format(dataset, epoch)
                submit(official_score_file, results, label_dict)
                accuracy = dev_metrics["ACC"]
            ...
            if test_data is not None:
                ...
                score_file = output_dir + "/" + "{}_test_scores_{}.json".format(dataset, epoch)
                results = {'metrics': test_metrics, 'predictions': test_predictions, 'uids': test_ids, 'scores': scores}
                dump(score_file, results)
                official_score_file = output_dir + "/" + "{}_test_scores_{}.tsv".format(dataset, epoch)
                submit(official_score_file, results, label_dict)
                logger.info('[new test scores saved.]')

        model_file = output_dir +"/" + "{}_model_{}.pt".format(dataset,epoch)
        model.save(model_file)
